#!/usr/bin/env python3
"""
Databricks Direct OAuth App - Confidential Client
Authenticates directly with Databricks OAuth endpoints and executes SQL queries
"""
...
app = Flask(__name__)
app.secret_key = os.environ.get('FLASK_SECRET_KEY', 'databricks-direct-oauth-app-secret')
...
class Config:
    def __init__(self):
        ...
        self.redirect_uri = os.environ.get('REDIRECT_URI', 'http://localhost:8001/callback')
        ...
        if not self.client_secret:
            logger.warning("Databricks client secret not configured")
...
def refresh_access_token():
    """Use refresh token to get new access token from Databricks"""
...
def login():
    ...
    try:
        ...
        logger.info("Redirecting to Databricks OAuth authorization")
...
def callback():
    ...
    try:
        ...
        logger.info("Databricks OAuth flow completed successfully")
        return redirect(url_for('sql_interface'))
...
if __name__ == '__main__':
    ...
    logger.info(f"Starting Databricks Direct OAuth App on port {port}")
